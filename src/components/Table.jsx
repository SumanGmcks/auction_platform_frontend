        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500">
          ...
          <tbody>
            {(Array.isArray(products) ? products : []).map((product) => (
              <tr className="bg-white border-b hover:bg-gray-50" key={product?._id || product?.id || Math.random()}>
                <td className="px-6 py-4">{product?.title?.slice(0, 15) ?? ""}...</td>
                <td className="px-6 py-4">{product?.comission ?? ""}%</td>
                ...
                {!isAdmin && (
                  <td className="px-6 py-4">
                    {product?.isSoldout ? (
                     ...
                     <button  onClick={() => handleSellProduct(product._id)} disabled={!product?.isVerify} className={` py-1 px-3 rounded-lg ${product?.isVerify? "bg-green text-white" :"bg-gray-300 text-black cursor-not-allowed"}`}>
                       Sell
                     </button>
                    )}
                  </td>
                ...
                <td className="px-6 py-4 text-center flex items-center gap-3 mt-1">
                  ...
                  <NavLink to={isAdmin ? `/product/admin/update/${product?._id}` : `/product/update/${product?._id}`} type="button" className="font-medium text-green">
                    <CiEdit size={25} />
                  </NavLink>
